...
// アプリ名＋バージョン
AppVerName=zLog 令和 Edition Version 3.0.2.0
...
AppCopyright=COPYRIGHT(C) 2026 JR8PPG
...
Name: "Options\Hamlog"; Description: "HAMLOGユーティリティー"; Types: full
...
Source: files\hamlog\zlog_hamlogconv.exe; DestDir: {app}; Components: "Options\Hamlog"; Flags: ignoreversion; Check: Is64BitInstallMode;
Source: files\hamlog\HamlogLookup.exe; DestDir: {app}; Components: "Options\Hamlog"; Flags: ignoreversion; Check: Is64BitInstallMode;
Source: files\hamlog\hamlog50.dll; DestDir: {app}; Components: "Options\Hamlog"; Flags: ignoreversion; Check: Is64BitInstallMode;
...
Source: files\hamlog\zlog_hamlogconv.exe; DestDir: {app}; Components: "Options\Hamlog"; Flags: ignoreversion; Check: not Is64BitInstallMode;
Source: files\hamlog\HamlogLookup.exe; DestDir: {app}; Components: "Options\Hamlog"; Flags: ignoreversion; Check: not Is64BitInstallMode;
Source: files\hamlog\hamlog50.dll; DestDir: {app}; Components: "Options\Hamlog"; Flags: ignoreversion; Check: not Is64BitInstallMode;
...
// Documents
Source: "files\zlog_v3020_release_note.pdf"; DestDir: {app}; Components: "Options\Documents";
...
var
  ...
  UserPage3: TInputOptionWizardPage;
  BakString: string;
  RootFolder: string;
  CfgDatFolder: string;
...
begin
   if CurStep = ssPostInstall then begin
      ...
      S8 := GetIniString('Preferences', 'BsResumePath', 'RESUME', ini);
      ...
      CreateDir(RootFolder + '\' + S8);
      
      if RootFolder = ExpandConstant('{app}') then begin
         SetIniString('Preferences', 'RootPath', '%ZLOG_ROOT%', ini);
      end
      else begin
      ...
      SetIniString('Preferences', 'BsResumePath', S8, ini);
      ...
      SetIniString('Profiles', 'IARUZone', S5, ini);
      
      // 操作方法
      if UserPage3.Values[0] = True then begin
         SetIniInt('OPERATE_STYLE', 'mode', 0, ini);
      end
      else begin
         SetIniInt('OPERATE_STYLE', 'mode', 1, ini);
      end;
   end;
end;
...
begin
   ...
   UserPage3 := CreateInputOptionPage(UserPage2.ID,
      'カスタマイズ', '操作方法', '操作方法を選択してください。（後で変更できます）', True, False);
   UserPage3.Add('オリジナルモード（標準）');
   UserPage3.Add('Ｅｎｔｅｒキーモード');
...
begin
   if CurPageID = wpSelectDir then begin
      ...
      // 操作方法
      n := GetIniInt('OPERATE_STYLE', 'mode', 0, 0, 1, ini);
      UserPage3.Values[n] := True;
   end;
